    ...
    if(count($_POST) == count($respostasCertas)){
        for($i = 0; $i < count($respostasCertas); $i++){
            if($respostasCertas[$i] == $_POST["resposta".($i+1)]){
                $cont++;
                echo "<script>document.querySelector('.pergunta" . ($i+1) . "').classList.add('certa');</script>";

            } else {
                #adicionar um atributo em uma classe
                echo "<script>document.querySelector('.pergunta" . ($i+1) . "').classList.add('errada');</script>";
            }
        }
    } else {
